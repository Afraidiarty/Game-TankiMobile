...
$query = "SELECT * FROM `buildings_user` WHERE id_user = '$myID' AND id_buildings = 2";
$result = mysqli_query($connect,$query);
$polygon = mysqli_fetch_assoc($result);

$currentLevel = $polygon['buildings_level'];
    ...
    if ($newLevel > 0 && $newLevel <= 16) {
        ...
        if ($result) {
            ...
            if (
                ...
                $query = "UPDATE `buildings_user` SET buildings_level = $newLevel+1 WHERE id_user = '$myID' AND id_buildings = 2";
                $updateResult = mysqli_query($connect, $query);

                $query_us = "UPDATE `users` SET silver = {$users['silver']}, ore = {$users['ore']}, gold = {$users['gold']}, iron = {$users['iron']}, steel = {$users['steel']}, plumbum = {$users['plumbum']} WHERE id = '$myID'";
...
if ($currentLevel < 16) {
    echo '<a class="simple-but border mb5" href="up-polygon?level=' . ($currentLevel) . '">';
